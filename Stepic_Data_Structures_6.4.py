...
class AVLTree:
    ...
    def edit_height(node: Node):
        ...
        while now_node is not None:
            ...
            if now_node.left_son is not None and now_node.right_son is not None \
                and now_node.left_son.height > 3 and now_node.right_son.height > 3 \
                and new_height == ex_height:
                break
            now_node.height = new_height
    ...
    @staticmethod
    def edit_sum(node: Node):
        now_node = node
        while now_node is not None:
            ex_sum = now_node.sum
            left_sum = now_node.left_son.sum if now_node.left_son is not None else 0
            right_sum = now_node.right_son.sum if now_node.right_son is not None else 0
            new_sum = left_sum + right_sum + now_node.value
            if now_node.height > 3 and new_sum == ex_sum:
                break
            now_node.sum = new_sum
            now_node = now_node.parent
    ...
    def split(self, key, root):
        ...
        self.roots.append(small_root)
        self.roots.append(big_root)
